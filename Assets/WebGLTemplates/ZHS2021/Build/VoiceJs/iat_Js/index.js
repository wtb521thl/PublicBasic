...
// 音频转码worker
let recorderWorker = new Worker('Build/VoiceJs/iat_Js/transformpcm.worker.js')
// 记录处理的缓存音频
let buffer = []
let buffertemp = []
...
recorderWorker.onmessage = function (e) {
    //buffertemp.push(...e.data.buffer)
    //buffer.push(...buffertemp.splice(0, 1280))
    //buffer = buffertemp.splice(0, 1280)
...
class IatRecorder {
    ...
    stop() {
        ...
        try {
            ...
            recorderWorker = new Worker('Build/VoiceJs/iat_Js/transformpcm.worker.js')
            recorderWorker.onmessage = function (e) {
                //buffertemp.push(...e.data.buffer)
                //buffer.push(...buffertemp.splice(0, 1280))
                //buffer = buffertemp.splice(0, 1280)
...
class IatTaste {
    ...
    setResult(data) {
        ...
        //window.gameInstance.SendMessage('VoiceManager', 'RecognitionCallBack', str);
        
        RecognitionCallBack(str);
        // resultStr = this.resultText + str
        // $('#result_output').text(resultStr)
    }
    setEndResult() {
        RecognitionEndCallBack();
        //var str = 'RecognitionEndCallBack';
        //VoiceCallBack(str);
        //window.gameInstance.SendMessage('VoiceManager', 'RecognitionEndCallBack', str);
        // resultStr = this.resultText + str
        // $('#result_output').text(resultStr)
    }
    counterDown() {
